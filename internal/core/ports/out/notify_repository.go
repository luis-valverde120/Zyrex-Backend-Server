...
type NotifyRepository interface {
	Save(notify *models.Notify) (*models.Notify, error)    // Save a new notify to the database
	Update(notify *models.Notify) (*models.Notify, error)  // Update an existing notify in the database
	Delete(id int) error                                   // Delete a notify by ID
	FindByOwnerID(ownerID int) ([]*models.Notify, error)   // Find notifies by owner ID
	FindByID(id int) (*models.Notify, error)               // Find a notify by ID
	FindByClientID(clientID int) ([]*models.Notify, error) // Find notifies by client ID
}
